...
当前状态：相关单测、类型检查与 ESLint 通过；最新 Electron 实机与真实支付验证待完成。历史截图不代表最新版本已验收。
...
三个目的分别组织提交；客户端界面与启动缓存分开保存为本地提交，尚未推送。服务端支付宝修复已通过独立 PR #701 合并；此前已合并的 Stripe 取消状态与退款账单关联修复不包含在上述范围中。启动缓存改动不属于套餐页面逻辑。
